...
class MSE_with_RelU:
    ...
    def forward_pass(self):          
        ...
        self.h = relU(self.pre_h)
        
        self.y_hat = self.W2 @ self.h + self.b2
    ...
    def backpropagate(self):        
        ...
        dh = self.W2.transpose() @ dy_hat
        dpre_h = dh * relU_derivative(self.pre_h)
    ...
    def log_to_files(self, f):
        ...
        print(f"Epoch {self.current_epoch} | Loss: {self.L} | Output: {np.vectorize(round_values)(self.y_hat)}\n")
        f.write(f"Epoch {self.current_epoch} | Loss: {self.L} | Output: {np.vectorize(round_values)(self.y_hat)}\n")
                    
class MSE_with_Sigmoid:
